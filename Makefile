...
GO_PACKAGES=./services/... ./indexer/... ./oduwacoin/... ./configuration/...
...
build:
	docker build -t rosetta-oduwacoin:latest https://github.com/ODUWAX/rosetta-oduwacoin.git
...
build-release:
	...
	docker build -t rosetta-oduwacoin:$(version) .;
	docker save rosetta-oduwacoin:$(version) | gzip > rosetta-oduwacoin-$(version).tar.gz;

run-mainnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/oduwacoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=MAINNET" -e "PORT=8080" -p 8080:8080 -p 26681:26681 rosetta-oduwacoin:latest

run-mainnet-offline:
	docker run -d --rm -e "MODE=OFFLINE" -e "NETWORK=MAINNET" -e "PORT=8081" -p 8081:8081 rosetta-oduwacoin:latest

run-testnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/oduwacoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=TESTNET" -e "PORT=8080" -p 8080:8080 -p 126681:126681 rosetta-oduwacoin:latest
