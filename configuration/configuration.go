...
const (
	...
	// Mainnet is the Oduwacoin Mainnet.
	Mainnet string = "MAINNET"

	// Testnet is Oduwacoin Testnet3.
	Testnet string = "TESTNET"

	// mainnetConfigPath is the path of the Oduwacoin
	// configuration file for mainnet.
	mainnetConfigPath = "/app/oduwacoin-mainnet.conf"

	// testnetConfigPath is the path of the Oduwacoin
	// configuration file for testnet.
	testnetConfigPath = "/app/oduwacoin-testnet.conf"
	...
	mainnetRPCPort = 26682
	testnetRPCPort = 126682

	// min prune depth is 288:
	// https://github.com/oduwacoin/oduwacoin/blob/ad2952d17a2af419a04256b10b53c7377f826a27/src/validation.h#L84
	pruneDepth = int64(10000) //nolint

	// min prune height (on mainnet):
	// https://github.com/oduwacoin/oduwacoin/blob/62d137ac3b701aae36c1aa3aa93a83fd6357fde6/src/chainparams.cpp#L102
...
func LoadConfiguration(baseDirectory string) (*Configuration, error) {
	...
	case Online:
		...
		config.OduwacoindPath = path.Join(baseDirectory, oduwacoindPath)
		if err := ensurePathExists(config.OduwacoindPath); err != nil {
			return nil, fmt.Errorf("%w: unable to create oduwacoind path", err)
		}
	case Offline:
		config.Mode = Offline
	case "":
	...
	case Mainnet:
		config.Network = &types.NetworkIdentifier{
			Blockchain: oduwacoin.Blockchain,
			Network:    oduwacoin.MainnetNetwork,
		}
		config.GenesisBlockIdentifier = oduwacoin.MainnetGenesisBlockIdentifier
		config.Params = oduwacoin.MainnetParams
		config.Currency = oduwacoin.MainnetCurrency
	...
	case Testnet:
		config.Network = &types.NetworkIdentifier{
			Blockchain: oduwacoin.Blockchain,
			Network:    oduwacoin.TestnetNetwork,
		}
		config.GenesisBlockIdentifier = oduwacoin.TestnetGenesisBlockIdentifier
		config.Params = oduwacoin.TestnetParams
		config.Currency = oduwacoin.TestnetCurrency
