	...
	tests := map[string]struct {
		...
		"all set (mainnet)": {
			...
			cfg: &Configuration{
				...
				Params:                 oduwacoin.MainnetParams,
				Currency:               oduwacoin.MainnetCurrency,
		...
		"all set (testnet)": {
			...
			cfg: &Configuration{
				...
				GenesisBlockIdentifier: oduwacoin.TestnetGenesisBlockIdentifier,
				Port:                   1000,
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if test.err != nil {
				...
				test.cfg.OduwacoindPath = path.Join(newDir, "oduwacoind")
