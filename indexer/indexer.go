...
import (
	...
	"github.com/ODUWAX/rosetta-oduwacoin/services"
...
type Client interface {
	...
	GetRawBlock(context.Context, *types.PartialBlockIdentifier) (*oduwacoin.Block, []string, error)
	ParseBlock(
		context.Context,
		*oduwacoin.Block,
...
func Initialize(
	...
	asserter, err := asserter.NewClientWithOptions(
		...
		oduwacoin.OperationTypes,
		oduwacoin.OperationStatuses,
		services.Errors,
		nil,
	)
	if err != nil {
...
// waitForNode returns once oduwacoind is ready to serve
// block queries.
func (i *Indexer) waitForNode(ctx context.Context) error {
	...
	for {
		...
		logger.Infow("waiting for oduwacoind...")
...
// Sync attempts to index Oduwacoin blocks using
// the oduwacoin.Client until stopped.
...
// Prune attempts to prune blocks in oduwacoind every
// pruneFrequency.
func (i *Indexer) Prune(ctx context.Context) error {
	...
	for {
		...
		case <-tc.C:
			...
			// Must meet pruning conditions in oduwacoin core
			// Source:
			// https://github.com/oduwacoin/oduwacoin/blob/a63a26f042134fa80356860c109edb25ac567552/src/rpc/blockchain.cpp#L953-L960
			...
			logger.Infow("attempting to prune oduwacoind", "prune height", pruneHeight)
			prunedHeight, err := i.client.PruneBlockchain(ctx, pruneHeight)
			if err != nil {
				logger.Warnw(
					"unable to prune oduwacoind",
					"prune height", pruneHeight,
					"error", err,
				)
			} else {
				logger.Infow("pruned oduwacoind", "prune height", prunedHeight)
			}
		}
	}
}
...
func (i *Indexer) findCoin(
	...
	for ctx.Err() == nil {
		...
		// multiple listeners)
		transactionHash := oduwacoin.TransactionHash(coinIdentifier)
...
func (i *Indexer) findCoins(
	...
	for _, coinIdentifier := range remainingCoins {
		// Wait on Channel
		txHash := oduwacoin.TransactionHash(coinIdentifier)
...
func (i *Indexer) GetScriptPubKeys(
	ctx context.Context,
	coins []*types.Coin,
) ([]*oduwacoin.ScriptPubKey, error) {
	databaseTransaction := i.database.ReadTransaction(ctx)
	defer databaseTransaction.Discard(ctx)

	scripts := make([]*oduwacoin.ScriptPubKey, len(coins))
	for j, coin := range coins {
		coinIdentifier := coin.CoinIdentifier
		transactionHash, networkIndex, err := oduwacoin.ParseCoinIdentifier(coinIdentifier)
		if err != nil {
		...
		for _, op := range transaction.Operations {
			if op.Type != oduwacoin.OutputOpType {
				continue
			}
			...
			var opMetadata oduwacoin.OperationMetadata
