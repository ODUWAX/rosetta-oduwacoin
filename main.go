...
import (
	...
	"github.com/ODUWAX/rosetta-oduwacoin/indexer"
...
func startOnlineDependencies(
	...
	g *errgroup.Group,
) (*oduwacoin.Client, *indexer.Indexer, error) {
	client := oduwacoin.NewClient(
		oduwacoin.LocalhostURL(cfg.RPCPort),
		cfg.GenesisBlockIdentifier,
		cfg.Currency,
	)

	g.Go(func() error {
		return oduwacoin.StartOduwacoind(ctx, cfg.ConfigPath, g)
	})
...
func main() {
	...
	if signalReceived {
		logger.Fatalw("rosetta-oduwacoin halted")
	}

	if err != nil {
		logger.Fatalw("rosetta-oduwacoin sync failed", "error", err)
	}
}
